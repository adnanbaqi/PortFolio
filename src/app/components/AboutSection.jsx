...
// TabButton component defined inline to avoid import issues
const TabButton = ({ active, selectTab, children }) => {
  const buttonClasses = active
    ? "text-white bg-gradient-to-r from-blue-500 to-purple-500 border-blue-500"
    : "text-slate-400 hover:text-white bg-slate-800/50 hover:bg-slate-700/50 border-slate-600";

  return (
    <button
      onClick={selectTab}
      className={`px-6 py-3 rounded-xl font-medium transition-all duration-300 border ${buttonClasses} flex-1 min-w-0`}
    >
      {children}
    </button>
  );
